...
fn main() -> Result<()> {
    ...
    let filter =
        EnvFilter::from_default_env().add_directive(format!("hostwatch={}", level).parse()?);
    tracing_subscriber::registry()
        .with(filter)
        .with(stdout_layer)
        .with(syslog_layer_opt)
        .init();

    if !args.interface.contains(&"any".to_string())
        && !args.filename.clone().unwrap_or("".to_string()).is_empty()
    {
    ...
    if args.foreground {
        ...
        let mut daemonize = Daemonize::new()
            ...
            .privileged_action(|| hostwatch.expect("Unable to initialize captures").init());
        ...
        match daemonize.start() {
            Ok(child) => child.run().expect("Failed to execute captures"),
